...
function RecommendationTextsPopover({
  ...
  // Превью следует за фокусом поля; при потере фокуса — карточка блюда.
...
export function UpsellWorkspace({
  ...
  return (
    <>
      <CatalogSidebar
        title="Рекомендации"
        description="Выберите позицию и настройте, что предложить вместе."
        showQuickActions={false}
      >
        <div className="mt-4 flex h-10 items-center rounded-xl border border-border px-3 text-sm text-zinc-400">
      ...
      <main className="min-w-0 flex-1 overflow-y-auto bg-white p-8">
        <div className="mx-auto max-w-4xl space-y-6">
          <PageTitle
            title="Рекомендации"
            description="Что предложить вместе с позициями и как это видит гость."
            showContentLanguage
            actionSlot={
