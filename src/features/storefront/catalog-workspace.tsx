...
function CatalogLocalViewTabs({
  ...
  onAllPositionsClick: () => void;
}) {
  const tabs = [
    { id: "sections" as const, label: "Разделы", icon: <List size={14} /> },
    { id: "all" as const, label: "Все позиции", icon: <Asterisk size={14} /> },
  ];

  return (
    <div className="flex h-8 w-full items-center rounded-[9px] bg-[#f0f0ea] p-[2px]">
      {tabs.map((tab) => {
        ...
        return (
          <button
            key={tab.id}
            type="button"
            onClick={tab.id === "sections" ? onSectionsClick : onAllPositionsClick}
            aria-pressed={selected}
            className={cn(
              "flex h-7 min-w-0 flex-1 items-center justify-center gap-1.5 rounded-[8px] px-2 text-[12px] leading-4 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#292524]/10",
              selected
                ? "border border-[#e7e5e4] bg-white font-medium text-[#292524] shadow-[0_1px_1px_rgba(0,0,0,0.05)]"
                : "border border-transparent font-normal text-[#79716b] hover:text-[#57534d]",
            )}
          >
            <span className={cn("flex h-3.5 w-3.5 shrink-0 items-center justify-center", selected ? "text-[#57534d]" : "text-[#79716b]")}>
              {tab.icon}
            </span>
            ...
            <DropdownMenu.Root>
              <DropdownMenu.Trigger asChild>
                <button
                  type="button"
                  className={cn(
                    "flex h-8 w-full items-center gap-2 rounded-[8px] border border-transparent px-[6px] pr-2 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#292524]/10",
